...
for RC in /home/aiuser/.bashrc /home/aiuser/.zshrc; do
    if [ -f "$RC" ]; then
        grep -q "ash-shell-init.sh" "$RC" 2>/dev/null || \
            echo -e '\n# Ash Shell — AI context hints\n[ -f /usr/share/ash-shell/ash-shell-init.sh ] && source /usr/share/ash-shell/ash-shell-init.sh' >> "$RC"
    fi
done
...
# 25. Setup ash-dev developer environment
/usr/lib/ash-dev/setup.sh
